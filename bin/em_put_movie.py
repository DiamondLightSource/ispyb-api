...
if __name__ == "__main__":
    ...
    parser.add_option(
        "--movieid", dest="movieid", help="Id for movie", metavar="INTEGER"
    )
    parser.add_option(
        "--mfile", dest="filename", help="Path to movie file", metavar="FILE"
    )
    parser.add_option("--visit", dest="visit", help="Visit name", metavar="STRING")
    parser.add_option(
        "--groupid", dest="groupid", help="Id for group", metavar="INTEGER"
    )
    parser.add_option(
    ...
    parser.add_option(
        "--microscopeid",
        dest="microscopeid",
        help="Id for microscope",
        metavar="INTEGER",
    )
    parser.add_option(
        "--micrograph",
        dest="micrograph",
        help="Path to micrograph image",
        metavar="FILE",
    )
    parser.add_option(
        "--powerspectrum1",
        dest="powerspectrum1",
        help="Path to 1st power spectrum image",
        metavar="FILE",
    )
    parser.add_option(
        "--powerspectrum2",
        dest="powerspectrum2",
        help="Path to 2nd power spectrum image",
        metavar="FILE",
    )
    parser.add_option(
        "--drift", dest="drift", help="Path to drift .dat file", metavar="FILE"
    )
    parser.add_option(
        "--nimages",
        dest="n_images",
        help="Number of images in .mrc file",
        metavar="INTEGER",
    )
    # parser.add_option("--framelen", dest="frame_len", help="Frame length (s)", metavar="FLOAT")
    parser.add_option(
        "--totexp", dest="tot_exp", help="Total exposure (s)", metavar="FLOAT"
    )
    parser.add_option(
        "--magnification", dest="magnification", help="(X)", metavar="FLOAT"
    )
    ...
    parser.add_option(
        "--totdose", dest="tot_dose", help="Total dose (e-/A^2)", metavar="FLOAT"
    )
    parser.add_option(
        "--runstatus", dest="run_status", help="Status of movie", metavar="STRING"
    )
    parser.add_option(
    ...
    parser.add_option(
        "--binning", dest="binning", help="Binning: 1 or 2 (X)", metavar="INTEGER"
    )
    parser.add_option(
        "--particlediameter",
        dest="particle_diameter",
        help="Particle diameter (nm)",
        metavar="FLOAT",
    )
    # parser.add_option("--pixelsize", dest="pixel_size", help="Pixel size (nm)", metavar="FLOAT")
    parser.add_option(
        "--boxsize", dest="box_size", help="Box size (nm)", metavar="FLOAT"
    )
    parser.add_option(
        "--minresol", dest="min_resol", help="Min resolution (A)", metavar="FLOAT"
    )
    parser.add_option(
        "--maxresol", dest="max_resol", help="Max resolution (A)", metavar="FLOAT"
    )
    parser.add_option(
        "--mindefocus", dest="min_defocus", help="Min defocus (A)", metavar="FLOAT"
    )
    parser.add_option(
        "--maxdefocus", dest="max_defocus", help="Max defocus (A)", metavar="FLOAT"
    )
    parser.add_option(
        "--defocusstepsize",
        dest="defocus_step_size",
        help="Defocus step size (A)",
        metavar="FLOAT",
    )
    parser.add_option(
        "--astigmatism",
        dest="astigmatism",
        help="Amount of astigmatism (A)",
        metavar="FLOAT",
    )
    parser.add_option(
        "--extractsize", dest="extract_size", help="Extract size (nm)", metavar="FLOAT"
    )
    parser.add_option(
        "--bgradius", dest="bg_radius", help="Background radius (nm)", metavar="FLOAT"
    )
    parser.add_option(
        "--stime",
        dest="stime",
    ...
    if visitid is not None or opts.movieid is not None:
        ...
        if opts.filename is not None:
            ...
            params["imgprefix"] = opts.filename[
                opts.filename.rfind("/") + 1 : opts.filename.rfind("_")
            ]
