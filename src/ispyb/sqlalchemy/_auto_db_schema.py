...
class AutoProc(Base):
    ...
    __table_args__ = (
        Index('AutoProc_refined_unit_cell', 'refinedCell_a', 'refinedCell_b', 'refinedCell_c', 'refinedCell_alpha', 'refinedCell_beta', 'refinedCell_gamma', 'spaceGroup'),
    )
...
class BLSampleType(Base):
    ...
    active = Column(TINYINT(1), server_default=text("1"), comment='1=active, 0=inactive')
...
class CryoemInitialModel(Base):
    ...
    ParticleClassification = relationship('ParticleClassification', secondary='ParticleClassification_has_CryoemInitialModel')
...
class Positioner(Base):
    ...
    __table_args__ = {'comment': 'An arbitrary positioner and its value, could be e.g. a motor. Allows for instance to store some positions with a sample or subsample'}
...
class ProcessingPipelineCategory(Base):
    ...
    __table_args__ = {'comment': 'A lookup table for the category of processing pipeline'}
...
class PurificationColumn(Base):
    ...
    __table_args__ = {'comment': 'Size exclusion chromotography (SEC) lookup table for BioSAXS'}
...
class BLSampleImageAutoScoreClass(Base):
    ...
    __table_args__ = {'comment': 'The automated scoring classes - the thing being scored'}

    blSampleImageAutoScoreClassId = Column(TINYINT(3), primary_key=True)
    blSampleImageAutoScoreSchemaId = Column(ForeignKey('BLSampleImageAutoScoreSchema.blSampleImageAutoScoreSchemaId', onupdate='CASCADE'), index=True)
...
class BeamLineSetup(Base):
    ...
    preferredDataCentre = Column(String(30), comment='Relevant datacentre to use to process data from this beamline')
...
class ProcessingPipeline(Base):
    ...
    __table_args__ = {'comment': 'A lookup table for different processing pipelines and their categories'}
    ...
    pipelineStatus = Column(Enum('automatic', 'optional', 'deprecated'), comment='Is the pipeline in operation or available')
    reprocessing = Column(TINYINT(1), server_default=text("1"), comment='Pipeline is available for re-processing')
...
class AutoProcScalingStatistics(Base):
    ...
    __table_args__ = (
        Index('AutoProcScalingStatistics_scalingId_statisticsType', 'autoProcScalingId', 'scalingStatisticsType'),
    )

    autoProcScalingStatisticsId = Column(INTEGER(10), primary_key=True, comment='Primary key (auto-incremented)')
    autoProcScalingId = Column(ForeignKey('AutoProcScaling.autoProcScalingId', ondelete='CASCADE', onupdate='CASCADE'), comment='Related autoProcScaling item (used by foreign key)')
...
class Phasing(Base):
    ...
    phasingProgramRunId = Column(ForeignKey('PhasingProgramRun.phasingProgramRunId', ondelete='CASCADE', onupdate='CASCADE'), nullable=False, index=True, comment='Related program item')
    spaceGroupId = Column(ForeignKey('SpaceGroup.spaceGroupId', ondelete='CASCADE', onupdate='CASCADE'), index=True, comment='Related spaceGroup')
    method = Column(Enum('solvent flattening', 'solvent flipping', 'e', 'SAD', 'shelxe'), comment='phasing method')
...
class BLSampleGroup(Base):
    ...
    proposalId = Column(ForeignKey('Proposal.proposalId', ondelete='SET NULL', onupdate='CASCADE'), index=True)
...
class BLSession(Base):
    ...
    proposalId = Column(ForeignKey('Proposal.proposalId', ondelete='CASCADE', onupdate='CASCADE'), nullable=False, server_default=text("0"))
...
class DiffractionPlan(Base):
    ...
    experimentKind = Column(Enum('Default', 'MXPressE', 'MXPressO', 'MXPressE_SAD', 'MXScore', 'MXPressM', 'MAD', 'SAD', 'Fixed', 'Ligand binding', 'Refinement', 'OSC', 'MAD - Inverse Beam', 'SAD - Inverse Beam', 'MESH', 'XFE', 'Stepped transmission', 'XChem High Symmetry', 'XChem Low Symmetry', 'Commissioning'))
    ...
    userPath = Column(String(100), comment='User-specified relative "root" path inside the session directory to be used for holding collected data')
    ...
    purificationColumnId = Column(ForeignKey('PurificationColumn.purificationColumnId'), index=True)
    collectionMode = Column(Enum('auto', 'manual'), comment='The requested collection mode, possible values are auto, manual')
    priority = Column(INTEGER(4), comment='The priority of this sample relative to others in the shipment')
    ...
    reductionParametersAveraging = Column(Enum('All', 'Fastest Dimension', '1D'), comment='Post processing params for SAXS')
...
class PhasingStatistics(Base):
    ...
    Phasing_has_Scaling = relationship('PhasingHasScaling', primaryjoin='PhasingStatistics.phasingHasScalingId1 == PhasingHasScaling.phasingHasScalingId')
    Phasing_has_Scaling1 = relationship('PhasingHasScaling', primaryjoin='PhasingStatistics.phasingHasScalingId2 == PhasingHasScaling.phasingHasScalingId')
...
class ProposalHasPerson(Base):
    ...
    role = Column(Enum('Co-Investigator', 'Principal Investigator', 'Alternate Contact', 'ERA Admin', 'Associate'))
...
class Protein(Base):
    ...
    description = Column(Text, comment='A description/summary using words and sentences')
    hazardGroup = Column(TINYINT(3), nullable=False, server_default=text("1"), comment='A.k.a. risk group')
    containmentLevel = Column(TINYINT(3), nullable=False, server_default=text("1"), comment='A.k.a. biosafety level, which indicates the level of containment required')
...
class SWOnceToken(Base):
    ...
    __table_args__ = {'comment': 'One-time use tokens needed for token auth in order to grant access to file downloads and webcams (and some images)'}
...
class DewarRegistry(Base):
    ...
    proposalId = Column(ForeignKey('Proposal.proposalId', onupdate='CASCADE'), index=True)
    labContactId = Column(ForeignKey('LabContact.labContactId', ondelete='SET NULL', onupdate='CASCADE'), index=True)
...
class SessionHasPerson(Base):
    ...
    sessionId = Column(ForeignKey('BLSession.sessionId', ondelete='CASCADE', onupdate='CASCADE'), primary_key=True, nullable=False, server_default=text("0"))
    ...
    role = Column(Enum('Local Contact', 'Local Contact 2', 'Staff', 'Team Leader', 'Co-Investigator', 'Principal Investigator', 'Alternate Contact', 'Data Access', 'Team Member', 'ERA Admin', 'Associate'))
...
class Dewar(Base):
    ...
    firstExperimentId = Column(ForeignKey('BLSession.sessionId', ondelete='SET NULL', onupdate='CASCADE'), index=True)
...
class DewarRegistryHasProposal(Base):
    ...
    proposalId = Column(ForeignKey('Proposal.proposalId'), index=True)
    personId = Column(ForeignKey('Person.personId'), index=True, comment='Person registering the dewar')
    recordTimestamp = Column(DateTime, server_default=text("current_timestamp()"))
    labContactId = Column(ForeignKey('LabContact.labContactId', onupdate='CASCADE'), index=True, comment='Owner of the dewar')
...
t_ShippingHasSession = Table(
    ...
    Column('shippingId', ForeignKey('Shipping.shippingId', ondelete='CASCADE', onupdate='CASCADE'), primary_key=True, nullable=False),
    Column('sessionId', ForeignKey('BLSession.sessionId', ondelete='CASCADE', onupdate='CASCADE'), primary_key=True, nullable=False, index=True)
)
...
class BFAutomationFault(Base):
    ...
    automationErrorId = Column(ForeignKey('BF_automationError.automationErrorId'), index=True)
    containerId = Column(ForeignKey('Container.containerId'), index=True)
    severity = Column(Enum('1', '2', '3'))
    stacktrace = Column(Text)
    resolved = Column(TINYINT(1))
    faultTimeStamp = Column(TIMESTAMP, nullable=False, server_default=text("current_timestamp()"))
...
class BLSample(Base):
    ...
    diffractionPlanId = Column(ForeignKey('DiffractionPlan.diffractionPlanId', ondelete='CASCADE', onupdate='CASCADE'), index=True)
    crystalId = Column(ForeignKey('Crystal.crystalId', ondelete='CASCADE', onupdate='CASCADE'))
...
class ContainerInspection(Base):
    ...
    __table_args__ = (
        Index('ContainerInspection_idx4', 'containerId', 'scheduleComponentid', 'state', 'manual'),
    )

    containerInspectionId = Column(INTEGER(11), primary_key=True)
    containerId = Column(ForeignKey('Container.containerId', ondelete='CASCADE', onupdate='CASCADE'), nullable=False)
...
class BLSampleImage(Base):
    ...
    blSampleImageScoreId = Column(ForeignKey('BLSampleImageScore.blSampleImageScoreId', onupdate='CASCADE'), index=True)
    comments = Column(String(255))
    blTimeStamp = Column(DateTime)
    ...
    offsetX = Column(INTEGER(11), nullable=False, server_default=text("0"), comment='The x offset of the image relative to the canvas')
    offsetY = Column(INTEGER(11), nullable=False, server_default=text("0"), comment='The y offset of the image relative to the canvas')
...
class BLSampleHasPositioner(Base):
    ...
    blSampleId = Column(ForeignKey('BLSample.blSampleId'), nullable=False, index=True)
    positionerId = Column(ForeignKey('Positioner.positionerId'), nullable=False, index=True)
...
class DataCollectionGroup(Base):
    ...
    __table_args__ = {'comment': 'a dataCollectionGroup is a group of dataCollection for a spe'}
...
class RobotAction(Base):
    ...
    actionType = Column(Enum('LOAD', 'UNLOAD', 'DISPOSE', 'STORE', 'WASH', 'ANNEAL', 'MOSAIC'))
    startTimestamp = Column(TIMESTAMP, nullable=False, server_default=text("current_timestamp() ON UPDATE current_timestamp()"))
    endTimestamp = Column(TIMESTAMP, nullable=False, server_default=text("'0000-00-00 00:00:00'"))
...
class XRFFluorescenceMappingROI(Base):
    ...
    edge = Column(String(15), comment='Edge type i.e. Ka1, could be a custom edge in case of overlap Ka1-noCa')
    ...
    blSampleId = Column(ForeignKey('BLSample.blSampleId'), index=True, comment='ROIs can be created within the context of a sample')
    scalar = Column(String(50), comment='For ROIs that are not an element, i.e. could be a scan counter instead')
...
class BLSampleImageHasAutoScoreClass(Base):
    ...
    __table_args__ = {'comment': 'Many-to-many relationship between drop images and thing being scored, as well as the actual probability (score) that the drop image contains that thing'}
...
class BLSubSample(Base):
    ...
    type = Column(String(10), comment='The type of subsample, i.e. roi (region), poi (point), loi (line)')
...
class BLSubSampleHasPositioner(Base):
    ...
    blSubSampleId = Column(ForeignKey('BLSubSample.blSubSampleId'), nullable=False, index=True)
    positionerId = Column(ForeignKey('Positioner.positionerId'), nullable=False, index=True)
...
class ContainerQueueSample(Base):
    ...
    status = Column(String(20), comment='The status of the queued item, i.e. skipped, reinspect. Completed / failed should be inferred from related DataCollection')
    ...
    dataCollectionPlanId = Column(ForeignKey('DiffractionPlan.diffractionPlanId'), index=True)
    blSampleId = Column(ForeignKey('BLSample.blSampleId'), index=True)
...
class DataCollection(Base):
    ...
    flux = Column(Float(asdecimal=True))
    strategySubWedgeOrigId = Column(INTEGER(10), index=True, comment='references ScreeningStrategySubWedge table')
    blSubSampleId = Column(ForeignKey('BLSubSample.blSubSampleId'), index=True)
...
class DataCollectionFileAttachment(Base):
    ...
    fileType = Column(Enum('snapshot', 'log', 'xy', 'recip', 'pia', 'warning', 'params'))
...
class ProcessingJobImageSweep(Base):
    ...
    __table_args__ = {'comment': 'This allows multiple sweeps per processing job for multi-xia2'}
...
class AutoProcProgramAttachment(Base):
    ...
    fileType = Column(Enum('Log', 'Result', 'Graph', 'Debug', 'Input'), comment='Type of file Attachment')
...
class Screening(Base):
    ...
    autoProcProgramId = Column(ForeignKey('AutoProcProgram.autoProcProgramId', ondelete='SET NULL', onupdate='CASCADE'), index=True)
...
class ZcZocaloBuffer(Base):
    ...
    AutoProcProgramID = Column(ForeignKey('AutoProcProgram.autoProcProgramId', ondelete='CASCADE', onupdate='CASCADE'), primary_key=True, nullable=False, comment='Reference to an existing AutoProcProgram')
    UUID = Column(INTEGER(10), primary_key=True, nullable=False, comment='AutoProcProgram-specific unique identifier')
    Reference = Column(INTEGER(10), comment='Context-dependent reference to primary key IDs in other ISPyB tables')
...
class AutoProcScalingHasInt(Base):
    ...
    autoProcScalingId = Column(ForeignKey('AutoProcScaling.autoProcScalingId', ondelete='CASCADE', onupdate='CASCADE'), comment='AutoProcScaling item')
...
class AutoProcStatus(Base):
    ...
    __table_args__ = {'comment': 'AutoProcStatus table is linked to AutoProcIntegration'}
...
class MXMRRunBlob(Base):
    ...
    filePath = Column(String(255), comment='File path corresponding to the filenames in the view* columns')
    ...
    nearestAtomResSeq = Column(MEDIUMINT(8), comment='Residue sequence number of nearest atom')
    nearestAtomDistance = Column(Float, comment='Distance in Angstrom to nearest atom')
    mapType = Column(Enum('anomalous', 'difference'), comment='Type of electron density map corresponding to this blob')
...
class ParticlePicker(Base):
    ...
    __table_args__ = {'comment': 'An instance of a particle picker program that was run'}

    particlePickerId = Column(INTEGER(10), primary_key=True)
    programId = Column(ForeignKey('AutoProcProgram.autoProcProgramId', onupdate='CASCADE'), index=True)
    firstMotionCorrectionId = Column(ForeignKey('MotionCorrection.motionCorrectionId', onupdate='CASCADE'), index=True)
    particlePickingTemplate = Column(String(255), comment='Cryolo model')
    particleDiameter = Column(Float, comment='Unit: nm')
    numberOfParticles = Column(INTEGER(10))
    summaryImageFullPath = Column(String(255), comment='Generated summary micrograph image with highlighted particles')
...
class RelativeIceThickness(Base):
    ...
    motionCorrectionId = Column(ForeignKey('MotionCorrection.motionCorrectionId', onupdate='CASCADE'), index=True)
    autoProcProgramId = Column(ForeignKey('AutoProcProgram.autoProcProgramId', onupdate='CASCADE'), index=True)
...
class XRFFluorescenceMapping(Base):
    ...
    __table_args__ = {'comment': 'An XRF map generated from an XRF Mapping ROI based on data from a gridscan of a sample'}
    ...
    dataFormat = Column(String(15), nullable=False, comment='Description of format and any compression, i.e. json+gzip for gzipped json')
    data = Column(LONGBLOB, nullable=False, comment='The actual data')
    points = Column(INTEGER(11), comment='The number of points available, for realtime feedback')
    opacity = Column(Float, nullable=False, server_default=text("1"), comment='Display opacity')
    ...
    autoProcProgramId = Column(ForeignKey('AutoProcProgram.autoProcProgramId'), index=True, comment='Related autoproc programid')
...
class ParticleClassificationGroup(Base):
    ...
    particlePickerId = Column(ForeignKey('ParticlePicker.particlePickerId', ondelete='CASCADE', onupdate='CASCADE'), index=True)
    programId = Column(ForeignKey('AutoProcProgram.autoProcProgramId', onupdate='CASCADE'), index=True)
    type = Column(Enum('2D', '3D'), comment='Indicates the type of particle classification')
    batchNumber = Column(INTEGER(10), comment='Corresponding to batch number')
    numberOfParticlesPerBatch = Column(INTEGER(10), comment='total number of particles per batch (a large integer)')
...
class XFEFluorescenceComposite(Base):
    ...
    __table_args__ = {'comment': 'A composite XRF map composed of three XRFFluorescenceMapping entries creating r, g, b layers'}

    xfeFluorescenceCompositeId = Column(INTEGER(10), primary_key=True)
    r = Column(ForeignKey('XRFFluorescenceMapping.xrfFluorescenceMappingId'), nullable=False, index=True, comment='Red layer')
    g = Column(ForeignKey('XRFFluorescenceMapping.xrfFluorescenceMappingId'), nullable=False, index=True, comment='Green layer')
    b = Column(ForeignKey('XRFFluorescenceMapping.xrfFluorescenceMappingId'), nullable=False, index=True, comment='Blue layer')
    rOpacity = Column(Float, nullable=False, server_default=text("1"), comment='Red layer opacity')
    bOpacity = Column(Float, nullable=False, server_default=text("1"), comment='Red layer opacity')
    gOpacity = Column(Float, nullable=False, server_default=text("1"), comment='Red layer opacity')
    opacity = Column(Float, nullable=False, server_default=text("1"), comment='Total map opacity')

    XRFFluorescenceMapping = relationship('XRFFluorescenceMapping', primaryjoin='XFEFluorescenceComposite.b == XRFFluorescenceMapping.xrfFluorescenceMappingId')
    XRFFluorescenceMapping1 = relationship('XRFFluorescenceMapping', primaryjoin='XFEFluorescenceComposite.g == XRFFluorescenceMapping.xrfFluorescenceMappingId')
    XRFFluorescenceMapping2 = relationship('XRFFluorescenceMapping', primaryjoin='XFEFluorescenceComposite.r == XRFFluorescenceMapping.xrfFluorescenceMappingId')
...
class ParticleClassification(Base):
    ...
    classNumber = Column(INTEGER(10), comment='Identified of the class. A unique ID given by Relion')
    classImageFullPath = Column(String(255), comment='The PNG of the class')
    particlesPerClass = Column(INTEGER(10), comment='Number of particles within the selected class, can then be used together with the total number above to calculate the percentage')
    ...
    particleClassificationGroupId = Column(ForeignKey('ParticleClassificationGroup.particleClassificationGroupId', ondelete='CASCADE', onupdate='CASCADE'), index=True)
    classDistribution = Column(Float, comment='Provides a figure of merit for the class, higher number is better')
...
t_ParticleClassification_has_CryoemInitialModel = Table(
    'ParticleClassification_has_CryoemInitialModel', metadata,
    Column('particleClassificationId', ForeignKey('ParticleClassification.particleClassificationId', ondelete='CASCADE', onupdate='CASCADE'), primary_key=True, nullable=False),
    Column('cryoemInitialModelId', ForeignKey('CryoemInitialModel.cryoemInitialModelId', ondelete='CASCADE', onupdate='CASCADE'), primary_key=True, nullable=False, index=True)
)
